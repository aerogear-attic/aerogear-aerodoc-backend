...
public class SaleAgentEndpoint
{
   ...
   {
      em.persist(entity);
      return Response.created(UriBuilder.fromResource(SaleAgentEndpoint.class).path(String.valueOf(entity.getId())).build()).build();
   }
   ...
   {
      TypedQuery<SaleAgent> findByIdQuery = em.createQuery("SELECT s FROM SaleAgent s WHERE s.id = :entityId", SaleAgent.class);
   ...
   {
      final List<SaleAgent> results = em.createQuery("SELECT s FROM SaleAgent s", SaleAgent.class).getResultList();
      return results;
   }
   ...
   public List<SaleAgent> listByCriteria(SaleAgentCriteria criteria){
	   
	   IdentityQuery<User> query =  identityManager.createIdentityQuery(User.class);
	   query.setParameter(IdentityType.ATTRIBUTE.byName("status"), new Object[] { criteria.getStatus() });
	   query.setParameter(IdentityType.ATTRIBUTE.byName("location"), new Object[] { criteria.getLocation() });
