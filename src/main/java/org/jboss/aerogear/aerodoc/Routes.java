...
public class Routes extends AbstractRoutingModule {
    ...
    private static final String[] JSON_MEDIA_TYPES = new String[] { "application/json; charset=utf-8", "application/json" };

    @Override
    public void configuration() throws Exception {
        route().on(AeroGearSecurityException.class).produces(JSON)
                .to(Error.class).index(param(HttpSecurityException.class));
        route().from("/login").on(RequestMethod.POST).consumes(JSON_MEDIA_TYPES)
                .produces(CustomMediaTypeResponder.CUSTOM_MEDIA_TYPE)
                .to(Login.class).login(param(SaleAgent.class));

        route().from("/logout").on(RequestMethod.POST).consumes(JSON_MEDIA_TYPES)
                .produces(CustomMediaTypeResponder.CUSTOM_MEDIA_TYPE)
                .to(Login.class).logout();

        route().from("/register").on(RequestMethod.POST).consumes(JSON_MEDIA_TYPES)
                .produces(CustomMediaTypeResponder.CUSTOM_MEDIA_TYPE)
                .to(Register.class).register(param(SaleAgent.class));

        route().from("/leads").roles("simple").on(RequestMethod.GET)
                .consumes(JSON_MEDIA_TYPES).produces(JSON).to(LeadEndpoint.class).listAll();

        route().from("/leads").roles("simple").on(RequestMethod.POST)
                .consumes(JSON_MEDIA_TYPES).produces(JSON).to(LeadEndpoint.class)
        ...
        route().from("/saleagents").roles("simple").on(RequestMethod.GET)
                .consumes(JSON_MEDIA_TYPES).produces(JSON).to(SaleAgentEndpoint.class)
                .listAll();
